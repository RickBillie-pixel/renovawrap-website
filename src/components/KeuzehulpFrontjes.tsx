...
export default function KeuzehulpFrontjes() {
  ...
  const uploadKeuzehulpFoto = async (file: File): Promise<string | null> => {
    ...
    const fileName = `keuzehulp-frontjes-${Date.now()}-${Math.random().toString(36).slice(2, 9)}.${fileExt}`;
  ...
  const handleSubmit = async () => {
    setSubmitting(true);
    setSubmitError(null);
    const serviceSlug: KeuzehulpServiceSlug = "frontjes";
    try {
  ...
  const steps = [
    { id: 1, title: "Type", label: "Type Front"},
    ...
    <div ref={sectionRef} className="flex flex-col lg:flex-row w-full max-w-7xl mx-auto min-h-[60vh] bg-background-light text-dark font-montserrat relative scroll-mt-8">
      ...
      <div className="lg:w-1/3 flex flex-col justify-start lg:sticky lg:top-24 h-auto lg:h-[60vh] mb-12 lg:mb-0 pr-8 lg:border-r border-gray-200 border-opacity-50">
        ...
        <h1 className="font-display text-3xl lg:text-4xl lg:text-5xl text-dark leading-tight mb-4">
          {step === 1 && <>Kies uw <br/> <span className="italic text-primary">type frontje</span></>}
          {step === 2 && <>Hoeveel <br/> <span className="italic text-primary">onderdelen?</span></>}
        ...
        <p className="text-gray-500 leading-relaxed mb-8 max-w-sm">
          {step === 1 && "Selecteer het type frontje dat het meeste lijkt op uw huidige keuken."}
        ...
        <AnimatePresence mode="wait">
          ...
          >
            ...
            {step === 2 && (
              <div className="grid grid-cols-2 md:grid-cols-2 gap-3 md:gap-6">
                 {[
                  { id: "1-10", label: "Tot 10 stuks", sub: "Kleine keuken", desc: "Bijvoorbeeld een rechte opstelling." },
                  { id: "10-20", label: "10 - 20 stuks", sub: "Gemiddeld", desc: "Standaard keukenopstelling." },
                  { id: "20-plus", label: "20+ stuks", sub: "Groot", desc: "Grote keuken met eiland." },
                  { id: "zakelijk", label: "Zakelijk / Project", sub: "Meerdere keukens", desc: "Voor projectontwikkeling of renovatie." },
                ].map((item) => (
                  <button
                    key={item.id}
