...
export default function KeuzehulpKozijnen() {
  ...
  const uploadKeuzehulpFoto = async (file: File): Promise<string | null> => {
    ...
    const fileName = `keuzehulp-kozijnen-${Date.now()}-${Math.random().toString(36).slice(2, 9)}.${fileExt}`;
  ...
  const handleSubmit = async () => {
    setSubmitting(true);
    setSubmitError(null);
    const serviceSlug: KeuzehulpServiceSlug = "kozijnen"; 
    try {
      ...
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="text-center p-12 bg-surface text-dark">
          ...
          <p className="text-gray-500 max-w-md mx-auto leading-relaxed">
            Wij nemen zo snel mogelijk contact met u op voor een vrijblijvend adviesgesprek en prijsopgave op maat voor uw kozijnen.
          </p>
        </div>
      </div>
    );
  ...
  const steps = [
    { id: 1, title: "Type", label: "Type Kozijn"},
    ...
    <div ref={sectionRef} className="flex flex-col lg:flex-row w-full max-w-7xl mx-auto min-h-[60vh] bg-background-light text-dark font-montserrat relative scroll-mt-8">
      ...
      <div className="lg:w-1/3 flex flex-col justify-start lg:sticky lg:top-24 h-auto lg:h-[60vh] mb-12 lg:mb-0 pr-8 lg:border-r border-gray-200 border-opacity-50">
        ...
        <h1 className="font-display text-3xl lg:text-4xl lg:text-5xl text-dark leading-tight mb-4">
          ...
          {step === 2 && <>Hoeveel <br/> <span className="italic text-primary">kozijnen?</span></>}
        ...
        <p className="text-gray-500 leading-relaxed mb-8 max-w-sm">
          {step === 1 && "Selecteer het type kozijn of deur."}
          {step === 2 && "Geef een indicatie van het aantal elementen."}
          {step === 3 && "Kies een kleur of structuur. Van houtlook tot modern mat."}
          {step === 4 && "Foto's helpen ons om de staat van de kozijnen te beoordelen."}
            ...
            {step === 2 && (
              <div className="grid grid-cols-2 md:grid-cols-2 gap-3 md:gap-6">
                 {[
                  ...
                  { id: "10-plus", label: "10+ Stuks", sub: "Groot", desc: "Volledige eengezinswoning." },
                  { id: "zakelijk", label: "Zakelijk", sub: "Projectmatig", desc: "Kantoorpand of bedrijfshal." },
                ].map((item) => (
                  <button
                    key={item.id}
            ...
            {step === 3 && (
              <div>
                <div className="relative mb-8">
                  ...
                  <input
                    type="text"
                    placeholder="Zoek op look (bijv. Antraciet, Hout, Zwart)..."
                    value={stijlSearch}
