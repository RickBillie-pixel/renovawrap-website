...
export default function KeuzehulpAfzuigkappen() {
  ...
  const uploadKeuzehulpFoto = async (file: File): Promise<string | null> => {
    ...
    const fileName = `keuzehulp-afzuigkappen-${Date.now()}-${Math.random().toString(36).slice(2, 9)}.${fileExt}`;
  ...
  const handleSubmit = async () => {
    setSubmitting(true);
    setSubmitError(null);
    const serviceSlug: KeuzehulpServiceSlug = "afzuigkappen" as any;
    try {
      ...
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="text-center p-12 bg-surface text-dark">
          ...
          <p className="text-gray-500 max-w-md mx-auto leading-relaxed">
            Wij nemen zo snel mogelijk contact met u op voor een vrijblijvend adviesgesprek en prijsopgave op maat voor uw afzuigkap.
          </p>
        </div>
      </div>
    );
    ...
    <div ref={sectionRef} className="flex flex-col lg:flex-row w-full max-w-7xl mx-auto min-h-[60vh] bg-background-light text-dark font-montserrat relative scroll-mt-8">
      ...
      <div className="lg:w-1/3 flex flex-col justify-start lg:sticky lg:top-24 h-auto lg:h-[60vh] mb-12 lg:mb-0 pr-8 lg:border-r border-gray-200 border-opacity-50">
        ...
        <h1 className="font-display text-3xl lg:text-4xl lg:text-5xl text-dark leading-tight mb-4">
          {step === 1 && <>Hoe breed is <br/> <span className="italic text-primary">uw afzuigkap?</span></>}
        ...
        <p className="text-gray-500 leading-relaxed mb-8 max-w-sm">
          {step === 1 && "Geef een schatting van de breedte van de afzuigkap."}
          ...
          >
            ...
            {step === 1 && (
              <div className="grid grid-cols-2 lg:grid-cols-2 gap-3 md:gap-6">
                 {[
                  { id: "60cm", label: "Tot 60 cm", sub: "Standaard", desc: "De meest voorkomende breedte." },
                  { id: "90cm", label: "90 cm", sub: "Breed", desc: "Medium of grote afzuigkappen." },
                  { id: "120plus", label: "120+ cm", sub: "Groot / Eiland", desc: "Grote schouw- of eilandkappen." },
