...
export default function KeuzehulpAanrechtbladen() {
  ...
  const canNext = () => {
    if (step === 1) return formData.vorm !== "";
  ...
  const uploadKeuzehulpFoto = async (file: File): Promise<string | null> => {
    ...
    const fileName = `keuzehulp-aanrecht-${Date.now()}-${Math.random().toString(36).slice(2, 9)}.${fileExt}`;
  ...
  const handleSubmit = async () => {
    setSubmitting(true);
    setSubmitError(null);
    const serviceSlug: KeuzehulpServiceSlug = "aanrechtbladen";
    try {
  ...
  const steps = [
    { id: 1, title: "Vorm", label: "Vorm"},
    { id: 2, title: "Afmeting", label: "Maten"},
    ...
    <div ref={sectionRef} className="flex flex-col lg:flex-row w-full max-w-7xl mx-auto min-h-[60vh] bg-background-light text-dark font-montserrat relative scroll-mt-8">
      ...
      <div className="lg:w-1/3 flex flex-col justify-start lg:sticky lg:top-24 h-auto lg:h-[60vh] mb-12 lg:mb-0 pr-8 lg:border-r border-gray-200 border-opacity-50">
        ...
        <h1 className="font-display text-3xl lg:text-4xl lg:text-5xl text-dark leading-tight mb-4">
          {step === 1 && <>Wat is de vorm <br/> <span className="italic text-primary">van uw blad?</span></>}
          {step === 2 && <>Schatting <br/> <span className="italic text-primary">totale lengte?</span></>}
        ...
        <p className="text-gray-500 leading-relaxed mb-8 max-w-sm">
          {step === 1 && "Kies de vorm die het beste overeenkomt met uw huidige keukenopstelling."}
        ...
        <AnimatePresence mode="wait">
          ...
          >
            ...
            {step === 2 && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                 {[
                  { id: "small", label: "Klein", sub: "Tot 3 meter", desc: "Compacte rechte keuken." },
                  { id: "medium", label: "Gemiddeld", sub: "3 - 5 meter", desc: "Standaard hoekkeuken." },
                  { id: "large", label: "Groot", sub: "5 - 7 meter", desc: "Ruime keuken met eiland." },
                  { id: "xl", label: "Extra Groot", sub: "7+ meter", desc: "Zeer uitgebreide opstelling." },
                ].map((item) => (
                  <button
                    key={item.id}
            ...
            {step === 3 && (
              <div>
                <div className="relative mb-8">
                  ...
                  <input
                    type="text"
                    placeholder="Zoek op look (bijv. Marmer, Beton)..."
                    value={stijlSearch}
