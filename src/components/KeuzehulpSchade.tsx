...
export default function KeuzehulpSchade() {
  ...
  const canNext = () => {
    if (step === 1) return formData.schade_type !== "";
    if (step === 2) return formData.oppervlak !== "";
    if (step === 3) return formData.urgentie !== "";
    if (step === 4) return formData.fotos.length > 0; // Photos are mandatory for damage repair
  ...
  const uploadKeuzehulpFoto = async (file: File): Promise<string | null> => {
    ...
    const fileName = `keuzehulp-schade-${Date.now()}-${Math.random().toString(36).slice(2, 9)}.${fileExt}`;
  ...
  const handleSubmit = async () => {
    setSubmitting(true);
    setSubmitError(null);
    const serviceSlug: KeuzehulpServiceSlug = "schadeherstel";
    try {
      ...
      const wizard_data = {
        schade_type: formData.schade_type,
        oppervlak: formData.oppervlak,
  ...
  const steps = [
    { id: 1, title: "Schade", label: "Type Schade"},
    { id: 2, title: "Object", label: "Object"},
    { id: 3, title: "Tijd", label: "Urgentie"},
    { id: 4, title: "Foto's", label: "Bewijs"},
    ...
    <div ref={sectionRef} className="flex flex-col lg:flex-row w-full max-w-7xl mx-auto min-h-[60vh] bg-background-light text-dark font-montserrat relative scroll-mt-8">
      ...
      <div className="lg:w-1/3 flex flex-col justify-start lg:sticky lg:top-24 h-auto lg:h-[60vh] mb-12 lg:mb-0 pr-8 lg:border-r border-gray-200 border-opacity-50">
        ...
        <h1 className="font-display text-3xl lg:text-4xl lg:text-5xl text-dark leading-tight mb-4">
          {step === 1 && <>Wat is de <br/> <span className="italic text-primary">schade?</span></>}
          {step === 2 && <>Waar zit <br/> <span className="italic text-primary">de schade?</span></>}
          {step === 3 && <>Wanneer <br/> <span className="italic text-primary">herstellen?</span></>}
          {step === 4 && <>Upload <br/> <span className="italic text-primary">duidelijke foto's</span></>}
          {step === 5 && <>Uw <br/> <span className="italic text-primary">gegevens</span></>}
        </h1>

        <p className="text-gray-500 leading-relaxed mb-8 max-w-sm">
          {step === 1 && "Selecteer het type schade dat het beste overeenkomt met uw situatie."}
          {step === 2 && "Is het een keuken, kozijn, deur of ander object?"}
          {step === 3 && "Heeft het spoed of is het esthetisch?"}
          {step === 4 && "Foto's zijn essentieel voor een goede beoordeling van de schade."}
          {step === 5 && "Ontvang een offerte voor uw verzekering of eigen gebruik."}
        </p>
            ...
            {step === 1 && (
              <div className="grid grid-cols-2 md:grid-cols-2 gap-3 md:gap-6">
                {[
                  ...
                  <button
                    key={item.id}
                    onClick={() => setFormData((p) => ({ ...p, schade_type: item.id }))}
            ...
            {step === 2 && (
              <div className="grid grid-cols-2 md:grid-cols-2 gap-3 md:gap-6">
                 {[
                  ...
                  { id: "deur", label: "Deur", sub: "Binnen- / Buitendeur", desc: "Krassen of stootschade." },
                  { id: "overig", label: "Overig", sub: "Meubels / Vloer", desc: "Tafelbladen, kasten, etc." },
                ].map((item) => (
                  <button
                    key={item.id}
            ...
            {step === 3 && (
              <div className="grid grid-cols-3 md:grid-cols-3 gap-2 md:gap-6">
                 {[
                  ...
                  { id: "geen-haast", label: "Geen Haast", sub: "Oriënterend", desc: "Ik wil graag advies.", icon: "info" },
                ].map((item) => (
                  <button
                    key={item.id}
            ...
            {step === 4 && (
              <div>
                ...
                >
                  ...
                  <p className="text-sm text-gray-500 mb-6">Voeg minimaal 1 foto toe (verplicht)</p>
              ...
              <div className="space-y-6 max-w-xl">
                 <div className="grid grid-cols-1 gap-6">
                    ...
                    <div className="space-y-1 pt-4">
                      <label className="text-xs font-bold uppercase tracking-widest text-gray-500">Toelichting Schade</label>
                      <textarea
                        ...
                        placeholder="Beschrijf de schade en de oorzaak (indien relevant voor verzekering)..."
                      />
                    </div>
                 </div>
                 {submitError && (
