...
export default function KeuzehulpKasten() {
  ...
  const uploadKeuzehulpFoto = async (file: File): Promise<string | null> => {
    ...
    const fileName = `keuzehulp-kasten-${Date.now()}-${Math.random().toString(36).slice(2, 9)}.${fileExt}`;
  ...
  const handleSubmit = async () => {
    setSubmitting(true);
    setSubmitError(null);
    const serviceSlug: KeuzehulpServiceSlug = "kasten"; 
    try {
      ...
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="text-center p-12 bg-surface text-dark">
          ...
          <p className="text-gray-500 max-w-md mx-auto leading-relaxed">
            Wij nemen zo snel mogelijk contact met u op voor een vrijblijvend adviesgesprek en prijsopgave op maat voor uw kastrenovatie.
          </p>
        </div>
      </div>
    );
  ...
  const steps = [
    { id: 1, title: "Type", label: "Type Kast"},
    { id: 2, title: "Afmeting", label: "Formaat"},
    ...
    <div ref={sectionRef} className="flex flex-col lg:flex-row w-full max-w-7xl mx-auto min-h-[60vh] bg-background-light text-dark font-montserrat relative scroll-mt-8">
      ...
      <div className="lg:w-1/3 flex flex-col justify-start lg:sticky lg:top-24 h-auto lg:h-[60vh] mb-12 lg:mb-0 pr-8 lg:border-r border-gray-200 border-opacity-50">
        ...
        <h1 className="font-display text-3xl lg:text-4xl lg:text-5xl text-dark leading-tight mb-4">
          {step === 1 && <>Om wat voor <br/> <span className="italic text-primary">kast gaat het?</span></>}
          {step === 2 && <>Schatting <br/> <span className="italic text-primary">afmeting?</span></>}
        ...
        <p className="text-gray-500 leading-relaxed mb-8 max-w-sm">
          {step === 1 && "Kies het type meubel dat u wilt laten wrappen."}
          {step === 2 && "Geef een indicatie van de breedte van de kast."}
          {step === 3 && "Kies uit onze unieke collectie. Van houtstructuur tot ultra-mat."}
          {step === 4 && "Foto's van de kast helpen ons de prijs nauwkeurig te berekenen."}
        ...
        <AnimatePresence mode="wait">
          ...
          >
            ...
            {step === 2 && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                 {[
                  { id: "small", label: "Klein", sub: "Tot 1.5 meter", desc: "Bijv. dressoir of smalle kast." },
                  { id: "medium", label: "Middel", sub: "1.5 - 3 meter", desc: "Standaard kledingkast." },
                  { id: "large", label: "Groot", sub: "3+ meter", desc: "Grote kastenwand of walk-in." },
                  { id: "custom", label: "Maatwerk", sub: "Afwijkend", desc: "Speciale vormen of meerdere kasten." },
                ].map((item) => (
                  <button
                    key={item.id}
                  ...
                  >
                    <div className="flex justify-between items-start mb-4">
                      ...
                      <div className={`w-6 h-6 rounded-full border border-gray-300 flex items-center justify-center transition-colors ${formData.afmeting === item.id ? 'border-primary bg-primary text-white' : 'group-hover:border-primary'}`}>
                        {formData.afmeting === item.id && <span className="material-symbols-outlined text-[14px]">check</span>}
                      </div>
                    </div>
            ...
            {step === 3 && (
              <div>
                <div className="relative mb-8">
                  ...
                  <input
                    type="text"
                    placeholder="Zoek op look (bijv. Hout, Mat, Zwart)..."
                    value={stijlSearch}
